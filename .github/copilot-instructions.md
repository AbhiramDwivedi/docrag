...
3. **Testing**  
   ...
   * Test resources should be placed in `backend/tests/resources/` directory.
4. **Dependencies**  
...
* **Backend Structure**  
  * All imports must use the new backend structure: `docquest.ingestion.*`, `docquest.querying.*`, `docquest.shared.*`, `interface.cli.*`
  * The main package is `docquest` located at `backend/src/docquest/`
  * Never use old import paths like `backend.*`, `agent.*`, `ingest.*`, `cli.*`
* **Extractor (`docquest/ingestion/extractors/`)**  
...
* **Vector store (`docquest/ingestion/storage/vector_store.py`)**  
...
* **Agent Framework (`docquest/querying/agents/`)**  
